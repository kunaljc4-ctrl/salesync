...
// ========================================================
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbz67srG4kZXoX1fbjb2GGS-8AbmYSvJYVr6Yk6Ki9gYo6djj2AMF-C1GOxeihQixsstgw/exec"
...
const specDefinitions = {
    ...
    headphone_type_usage: {
        label: "Headphone Type",
        list: ["Office", "Gaming", "Mix"]
    },
    earbud_type: {
        label: "Earbud Type",
        list: ["In-ear", "Neckband"]
    }
};
...
const specRules = {
    ...
    Earbuds: [
        "battery", "earbud_type", "anc", "playtime", "connectivity"
    ],
    Headphones: [
        "headphone_type_usage", "anc", "playtime", "connectivity"
    ],
    Accessory: [
        "color"
    ],
...
function clearProductFields() {
    ...
    // Apply default GST
    const defaultGst = localStorage.getItem("defaultGst") || "18";
    if (itemTaxRate) itemTaxRate.value = defaultGst;
}
...
function generateInvoiceID() {
    ...
    let prefix = localStorage.getItem("invoicePrefix") || "INV-";
    let id = `${prefix}${datePart}-${String(counter).padStart(4, "0")}`;
    invEl.value = id;
}

// Initial defaults
if (globalPurchaseDate) globalPurchaseDate.valueAsDate = new Date();
const __gstInp = document.getElementById("itemTaxRate");
if (__gstInp && !__gstInp.value) __gstInp.value = localStorage.getItem("defaultGst") || "18";
generateInvoiceID();
...
// Helper to switch tab and focus a missing field
function focusField(id, tabIndex) {
    if (typeof tabs !== 'undefined' && tabs[tabIndex]) {
        tabs[tabIndex].click();
    }
    ...
    if (el) {
        setTimeout(() => {
            el.focus();
            showToast(`⚠ Required: ${el.labels?.[0]?.textContent || id}`);
        }, 300); // 300ms delay to allow tab transition
    }
}
...
async function submitToSheets() {
    ...
    // Mandatory Validation with Redirection
    if (!isDevMode) {
        const cName = document.getElementById("customerName")?.value.trim();
        const cPhone = document.getElementById("customerPhone")?.value.trim();
        const pMode = document.getElementById("paymentMode")?.value;
        const pStatusField = document.getElementById("paymentStatusField");
        const pStatus = document.getElementById("paymentStatus")?.value;

        if (!cName) {
            focusField("customerName", 0);
            return;
        }
        if (!cPhone || cPhone.length < 10) {
            focusField("customerPhone", 0);
            if (!cPhone) showToast("⚠ Please enter a Phone Number!");
            else showToast("⚠ Please enter a valid 10-digit Phone Number!");
            return;
        }
        if (!pMode) {
            focusField("paymentMode", 3);
            return;
        }
        if (pStatusField && pStatusField.style.display !== "none" && !pStatus) {
            focusField("paymentStatus", 3);
            return;
        }
    }

    const saveBtn = document.getElementById("saveToSheetsBtn");
    // Prevent double clicks (Robust Error Handling)
    ...
    console.log("SHIPPING DATA TO SHEETS (or LocalStorage):", payload);
    ...
    const completeSaleProcess = () => {
        ...
        if (waCheckbox && waCheckbox.checked) {
            if (typeof sendWhatsAppInvoice === 'function') sendWhatsAppInvoice();
        }
    ...
    // --- Offline Handling Phase (EVO 4) ---
    if (!navigator.onLine) {
        let pending = JSON.parse(localStorage.getItem('pendingSales') || '[]');
        pending.push(payload);
        localStorage.setItem('pendingSales', JSON.stringify(pending));

        showToast("⚠ No Internet! Sale saved locally. It will sync automatically when online.");
        completeSaleProcess();
        return;
    }

    // --- Online Try/Catch Network Phase (EVO 4) ---
    try {
        ...
        console.error("GS ERROR / API FAILURE:", err);
        // Fallback: API failed despite being "online" (server drop, bad proxy, etc.)
        let pending = JSON.parse(localStorage.getItem('pendingSales') || '[]');
        pending.push(payload);
        localStorage.setItem('pendingSales', JSON.stringify(pending));

        showToast("❌ Server API Failed! Sale saved locally as backup.");
        completeSaleProcess();
    }
}
...
function generatePDF() {
    ...
    const bName = localStorage.getItem("shopName") || "PREMIUM ELECTRONICS";
    const bAddress = localStorage.getItem("shopAddress") || "123 Tech Street, Digital City | +91 98765 43210";
    const bGst = localStorage.getItem("gstNumber") || "";
    const bLogo = localStorage.getItem("shopLogo");
    const invFormat = localStorage.getItem("invoiceFormat") || "simple";

    // --- 1. BRAND HEADER ---
    let headerX = 15;
    if (bLogo) {
        try {
            // Basic detection of image type from data URL
            let format = 'PNG';
            if (bLogo.includes('image/jpeg') || bLogo.includes('image/jpg')) format = 'JPEG';
            
            // Render logo at a fixed height of 20 units and calculate width
            // This is a simplified approach; true aspect ratio detection would require an Image object
            pdf.addImage(bLogo, format, 15, 12, 25, 25, undefined, 'FAST'); 
            headerX = 45; // Shift text to the right
        } catch (e) {
    ...
    pdf.setFontSize(26);
    pdf.text(bName, headerX, 25);

    pdf.setFontSize(10);
    pdf.setFont("helvetica", "normal");
    pdf.setTextColor(...slateGray);
    pdf.text(bAddress, headerX, 32);
    ...
    if (bGst) {
        ...
        pdf.text(`Merchant GST: ${bGst}`, 15, y);
        y += 5;
    }
...
if (resetBtn) {
    resetBtn.addEventListener("click", resetFullInvoiceForm);
}

function resetFullInvoiceForm() {
    if (confirm("Are you sure you want to clear EVERYTHING and start a new sale?")) {
        localStorage.removeItem("salesFormDraft");
        localStorage.removeItem("productListDraft");
        location.reload(); // Hard reset ensures pure state
    }
}
    ...
    resetSearchBtn.addEventListener("click", () => {
        ...
        if (resultsDiv) {
            resultsDiv.innerHTML = `
                <div style="text-align: center; color: var(--text-muted); padding: 60px 20px; background: var(--bg); border-radius: var(--radius-md); border: 1px dashed var(--border);">
                    <i class="fa-solid fa-server" style="font-size: 40px; margin-bottom: 16px; display: block; color: var(--primary); opacity: 0.5;"></i>
                    Enter a search query to pull records from the central database.
                </div>`;
        }
...
function renderSearchResults(results) {
    ...
    results.forEach(inv => {
        ...
        const custName = getInvVal(inv, "Customer name", "Name", "Full Name", "customer", "Customer Name", "Customer") || "N/A";
        const custPhone = getInvVal(inv, "Phone number", "phone", "Mobile", "WhatsApp", "Customer Phone", "Phone") || "N/A";
        ...
        card.innerHTML = `
            <div>
                ...
                <div class="s-name" style="cursor: pointer; color: var(--primary);" onclick="jumpToCustomerProfile('${custName.replace(/'/g, "\\'")}', '${custPhone}')">${custName}</div>
...
function reprintFromSearch(invData) {
    ...
    document.getElementById("customerAddress").value = getInvVal(invData, "Billing address", "Billing Address", "Billing", "Address", "Permanent address", "Customer Address", "Location", "City");
...
function renderAdvancedCharts(cData) {
    ...
    // 3. Units Sold by Products (Horizontal Bar)
    ...
    if (ctx5) {
        revenueQuarterlyChartObj = new Chart(ctx5, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Units',
                    data: cData.productsUnits.map(x => x[1]),
...
// ========================================================
// EVO 4: OFFLINE MODE AUTO-SYNC BACKGROUND JOB
// ========================================================
async function syncPendingSales() {
    ...
    console.log(`[Evo 4] Attempting to background sync ${pending.length} pending offline sales...`);
    ...
    try {
        for (let i = 0; i < pending.length; i++) {
            ...
            console.log("Synced pending sale " + (i + 1));
        }

        // Only clear if all fetched without throwing network errors
        localStorage.removeItem('pendingSales');
        showToast("📶 Offline sales successfully synced to cloud.");
...
// Trigger background sync on page load and online event
window.addEventListener('load', () => {
    syncPendingSales();
    setTimeout(() => {
        if (typeof checkNotifications === 'function') checkNotifications();
    }, 2000); // Give it a couple seconds for inventory and analytics to load
});

window.addEventListener('online', () => {
    showToast("📶 Internet Restored! Syncing pending sales...");
    syncPendingSales();
});

window.addEventListener('offline', () => {
    showToast("⚠ You are offline! Sales will be safely cached locally.");
});



// ========================================================
// EVO 4: CRM GOOGLE SHEETS SYNC
// ========================================================
let globalCrmData = [];
let globalCrmRawSales = [];

async function loadCRM() {
    const tbody = document.getElementById("crmTableBody");
    if (!tbody) return;

    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 40px;"><i class="fa-solid fa-spinner fa-spin" style="font-size: 24px;"></i><br>Syncing CRM from Google Sheets...</td></tr>';

    try {
        const res = await fetch(`${SCRIPT_URL}?action=getAnalytics`);
        const data = await res.json();

        if (data.success && data.results && data.results.length > 0) {

            globalCrmRawSales = data.results;

            // Map raw rows into unique customers
            const customers = {};

            data.results.forEach(row => {
                const cName = getInvVal(row, "Customer name", "Name", "Full Name", "customer", "Customer Name", "Customer");
                const phone = String(getInvVal(row, "Phone number", "phone", "Mobile", "WhatsApp", "Customer Phone", "Phone")).trim();
                if ((!cName || cName === "N/A") && (!phone || phone === "N/A")) return;

                const id = (phone && phone !== "N/A") ? phone : cName;
                ...
                if (!customers[id]) {
                    customers[id] = {
                        ...
                        visits: 0,
                        tags: [],
                        address: getInvVal(row, "Billing address", "Billing Address", "Billing", "Address", "Permanent address", "Customer Address", "Location", "City"),
                    };
                } else if (!customers[id].address) {
                    customers[id].address = getInvVal(row, "Billing address", "Billing Address", "Billing", "Address", "Permanent address", "Customer Address", "Location", "City");
                }

                const spend = Number(row["Grand Total"] || row["Subtotal"] || row["Total"] || 0);
                customers[id].totalSpend += spend;
                customers[id].visits += 1;

                const pMode = getInvVal(row, "paymentMode", "PaymentMode", "Payment Mode", "Payment_Mode", "Payment", "Payment method", "Mode", "Pay Mode", "Type", "Sales Mode");
                if (String(pMode).toLowerCase().includes("emi")) {
                    if (!customers[id].tags.includes("EMI Active")) customers[id].tags.push("EMI Active");
                }

                const dateStr = row["Purchase date"] || row["Date"] || row["timestamp"];
                if (dateStr) {
                    const rowDate = parseSheetDate(dateStr);
                    if (rowDate > customers[id].lastVisit) {
                        customers[id].lastVisit = rowDate;
                    }
                }
            });

            globalCrmData = Object.values(customers).sort((a, b) => b.lastVisit - a.lastVisit);

            const today = new Date();
            // Generate automatic tags
            globalCrmData.forEach(c => {
                const daysSinceVisit = Math.floor((today - c.lastVisit) / (1000 * 60 * 60 * 24));

                // 1. Credit (Previously EMI Active)
                // Already added during row loop if EMI was detected

                // 2. VIP
                if (c.totalSpend > 20000 && !c.tags.includes("VIP")) c.tags.push("VIP");

                // 3. High-Value
                if (c.totalSpend >= 15000 && c.visits <= 2 && !c.tags.includes("High-Value")) c.tags.push("High-Value");

                // 4. Frequent
                if (c.visits > 3 && !c.tags.includes("Frequent")) c.tags.push("Frequent");

                // 5. Returning
                if (c.visits > 1 && c.visits <= 3 && !c.tags.includes("Returning")) c.tags.push("Returning");

                // 6. New
                if (c.visits === 1 && !c.tags.includes("New")) c.tags.push("New");

                // 7. Inactive
                if (daysSinceVisit > 90 && !c.tags.includes("Inactive")) c.tags.push("Inactive");

                // 8. Risky
                // A customer with credit history who hasn't visited in 30+ days
                if (c.tags.includes("EMI Active") && daysSinceVisit > 30 && !c.tags.includes("Risky")) {
                    c.tags.push("Risky");
                }

                // Clean up tag names if needed (e.g. EMI Active -> Credit)
                c.tags = c.tags.map(t => t === "EMI Active" ? "Credit" : t);
            });

            renderCRMTable(globalCrmData);

        } else {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 40px; color: var(--text-muted);">No customers found in database.</td></tr>';
        }
    } catch (err) {
        console.error("CRM Load Error:", err);
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 40px; color: var(--text-muted);"><i class="fa-solid fa-triangle-exclamation" style="color: #ef4444; font-size: 24px; margin-bottom: 12px;"></i><br>Failed to sync CRM data from Cloud. Check connection.</td></tr>';
    }
}

function renderCRMTable(dataList) {
    const tbody = document.getElementById("crmTableBody");
    if (!tbody) return;

    if (dataList.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 40px; color: var(--text-muted);">No results match your search.</td></tr>';
        return;
    }

    const today = new Date();

    const formatDaysAgo = (dateDate) => {
        if (!dateDate || dateDate.getTime() === 0) return "Unknown";
        const diffTime = Math.abs(today - dateDate);
    ...
    tbody.innerHTML = dataList.map((c, i) => {

        const tagsHtml = c.tags.map(t => {
            let colors = { bg: 'rgba(100, 116, 139, 0.1)', fg: '#64748b' };
            if (t === 'VIP') colors = { bg: 'rgba(16, 185, 129, 0.1)', fg: '#10b981' };
            if (t === 'Credit') colors = { bg: 'rgba(245, 158, 11, 0.1)', fg: '#f59e0b' };
            if (t === 'New') colors = { bg: 'rgba(59, 130, 246, 0.1)', fg: '#3b82f6' };
            if (t === 'Returning') colors = { bg: 'rgba(139, 92, 246, 0.1)', fg: '#8b5cf6' };
            if (t === 'Frequent') colors = { bg: 'rgba(236, 72, 153, 0.1)', fg: '#ec4899' };
            if (t === 'High-Value') colors = { bg: 'rgba(20, 184, 166, 0.1)', fg: '#14b8a6' };
            if (t === 'Inactive') colors = { bg: 'rgba(107, 114, 128, 0.1)', fg: '#6b7280' };
            if (t === 'Risky') colors = { bg: 'rgba(239, 68, 68, 0.1)', fg: '#ef4444' };

            return `<span style="background: ${colors.bg}; color: ${colors.fg}; padding: 4px 10px; border-radius: 100px; font-size: 11px; font-weight: 600; white-space: nowrap;">${t}</span>`;
        }).join(" ");

        return `
        <tr style="cursor: pointer;" onclick="openCRMProfile('${c.id.replace(/'/g, "\\'")}')">
            <td>
                <div style="font-weight: 600; color: var(--text);">${c.name}</div>
                <div style="font-size: 12px; color: var(--text-muted);">${c.visits} Total Visits</div>
            </td>
            <td>${c.phone}</td>
            <td>${tagsHtml}</td>
            <td style="font-weight: 600;">₹${c.totalSpend.toLocaleString('en-IN')}</td>
            <td style="font-size: 13px; color: var(--text-muted);">${formatDaysAgo(c.lastVisit)}</td>
            <td><button class="btn-pill outline" style="padding: 4px 12px; font-size: 12px;">View</button></td>
        </tr>
        `;
    }).join('');
}
...
function openCRMProfile(customerId) {
    const c = globalCrmData.find(cust => cust.id === customerId);
    if (!c) return;

    // Populate identity
    document.getElementById('crmProfileName').innerText = c.name;
    document.getElementById('crmProfilePhone').innerText = c.phone;

    // Tags
    const tagsHtml = c.tags.map(t => {
        let colors = { bg: 'rgba(100, 116, 139, 0.1)', fg: '#64748b', icon: '' };
        if (t === 'VIP') colors = { bg: 'rgba(16, 185, 129, 0.1)', fg: '#10b981', icon: '<i class="fa-solid fa-crown"></i> ' };
        if (t === 'Credit') colors = { bg: 'rgba(245, 158, 11, 0.1)', fg: '#f59e0b', icon: '<i class="fa-solid fa-hand-holding-dollar"></i> ' };
        if (t === 'Risky') colors = { bg: 'rgba(239, 68, 68, 0.1)', fg: '#ef4444', icon: '<i class="fa-solid fa-triangle-exclamation"></i> ' };
        if (t === 'New') colors = { bg: 'rgba(59, 130, 246, 0.1)', fg: '#3b82f6', icon: '' };
        if (t === 'Returning') colors = { bg: 'rgba(139, 92, 246, 0.1)', fg: '#8b5cf6', icon: '' };
        if (t === 'Frequent') colors = { bg: 'rgba(236, 72, 153, 0.1)', fg: '#ec4899', icon: '' };
        if (t === 'High-Value') colors = { bg: 'rgba(20, 184, 166, 0.1)', fg: '#14b8a6', icon: '' };
        if (t === 'Inactive') colors = { bg: 'rgba(107, 114, 128, 0.1)', fg: '#6b7280', icon: '' };

        return `<span style="background: ${colors.bg}; color: ${colors.fg}; padding: 4px 10px; border-radius: 100px; font-size: 11px; font-weight: 700;">${colors.icon}${t}</span>`;
    }).join(" ");
    document.getElementById('crmProfileTags').innerHTML = tagsHtml;

    // We don't fetch address reliably from globalCrmRawSales right now, but we can look for the most recent address.
    const addressDiv = document.getElementById('crmProfileAddress');
    const notesDiv = document.getElementById('crmProfileNotes');
    let recentSale = null;

    if (globalCrmRawSales && globalCrmRawSales.length > 0) {
        recentSale = globalCrmRawSales.find(row => {
            const rowPhone = String(getInvVal(row, "Customer Phone", "Phone", "Mobile")).trim();
            const rowName = String(getInvVal(row, "Customer Name", "Customer", "Name")).trim();
            const searchPhone = String(c.phone).trim();
            const searchName = String(c.name).toLowerCase().trim();
            return (rowPhone && rowPhone === searchPhone) || (rowName && rowName.toLowerCase() === searchName);
        });
    }

    const addr = (recentSale ? getInvVal(recentSale, "Billing address", "Billing Address", "Billing", "Address", "Permanent address", "Customer Address", "Location", "City") : "") || c.address || "";
    if (addressDiv) {
        addressDiv.innerHTML = `<i class="fa-solid fa-location-dot" style="width: 20px; color: var(--text-muted); text-align: center;"></i> <strong>Billing Address:</strong> ${addr || "Not Provided"}`;
        addressDiv.style.display = 'block';
    }

    let generatedNote = "";
    const daysSinceVisit = Math.floor((new Date() - c.lastVisit) / (1000 * 60 * 60 * 24));

    if (c.tags.includes("VIP")) {
        generatedNote = "<strong>VIP Customer:</strong> High total spending and consistent payment history.";
    } else if (c.tags.includes("Risky")) {
        generatedNote = "<strong>Risky Customer:</strong> Outstanding credit balance with no recent visits.";
    } else if (c.tags.includes("Credit")) {
        generatedNote = "<strong>Credit Customer:</strong> Has pending payments or uses EMI finance.";
    } else if (c.tags.includes("High-Value")) {
        generatedNote = "<strong>High-Value Customer:</strong> Large spending transactions even if not frequent.";
    } else if (c.tags.includes("Frequent")) {
        generatedNote = "<strong>Frequent Customer:</strong> Regularly purchases items from the store.";
    } else if (c.tags.includes("Returning")) {
        generatedNote = "<strong>Returning Customer:</strong> Has visited and purchased multiple times.";
    } else if (c.tags.includes("New")) {
        generatedNote = "<strong>New Customer:</strong> First-time buyer with no prior history.";
    } else if (c.tags.includes("Inactive")) {
        generatedNote = "<strong>Inactive Customer:</strong> Has not made a purchase in over 90 days.";
    } else {
        generatedNote = "<strong>General Customer:</strong> Occasional visitor with average spending.";
    }

    if (notesDiv) {
        notesDiv.innerHTML = `<i class="fa-solid fa-note-sticky" style="width: 20px; color: var(--text-muted); text-align: center;"></i> ${generatedNote}`;
        notesDiv.style.display = 'block';
    }

    const btnWa = document.getElementById('crmActionWifi');
    if (btnWa) {
        btnWa.onclick = () => window.open(`https://wa.me/91${String(c.phone).replace(/[^0-9]/g, '')}`, '_blank');
    }

    const btnCall = document.getElementById('crmActionCall');
    if (btnCall) {
        btnCall.onclick = () => window.open(`tel:${String(c.phone)}`, '_self');
    }

    const btnInv = document.getElementById('crmActionInvoice');
    if (btnInv) {
        btnInv.onclick = () => {
            showToast("Switching to New Invoice");
            document.querySelector('[data-view="view-invoice"]').click();

            // Prefill invoice data - ONLY mobile number per user request
            const phoneEl = document.getElementById('customerPhone');
            if (phoneEl) phoneEl.value = c.phone !== "N/A" ? c.phone : "";
        };
    }

    const btnPay = document.getElementById('crmActionPayment');
    if (btnPay) btnPay.onclick = () => showToast("Payment collections are recorded via Sales.");

    // Purchase History table
    const tbody = document.getElementById('crmPurchaseHistory');

    if (globalCrmRawSales && globalCrmRawSales.length > 0) {
        const customerSales = globalCrmRawSales.filter(row => {
            const rowPhone = String(getInvVal(row, "Phone number", "phone", "Mobile", "WhatsApp", "Customer Phone", "Phone")).trim();
            const rowName = String(getInvVal(row, "Customer name", "Name", "Full Name", "customer", "Customer Name", "Customer")).trim();
            const searchPhone = String(c.phone).trim();
            const searchName = String(c.name).toLowerCase().trim();
            return (rowPhone !== "N/A" && rowPhone === searchPhone) || (rowName !== "N/A" && rowName.toLowerCase() === searchName);
        });

        customerSales.sort((a, b) => parseSheetDate(b["Purchase date"] || b["Date"] || b["timestamp"]) - parseSheetDate(a["Purchase date"] || a["Date"] || a["timestamp"]));

        if (customerSales.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align: center; color: var(--text-muted); padding: 20px;">No purchases found.</td></tr>';
        } else {
            tbody.innerHTML = customerSales.map(sale => {
                const dateRaw = sale["Purchase date"] || sale["Date"] || sale["timestamp"];
                const dateObj = parseSheetDate(dateRaw);
                const dStr = dateObj ? dateObj.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : "-";

                const invNo = getInvVal(sale, "Invoice ID", "invoiceId", "Invoice id", "ID", "Invoice Number", "Invoice No", "Invoice", "invoice");
                const items = getInvVal(sale, "Item", "Item Name", "Model", "Models", "Products", "Product Details", "Items", "Brand");
                const amount = Number(sale["Grand Total"] || sale["Subtotal"] || sale["Total"] || 0);
                const pMode = getInvVal(sale, "paymentMode", "PaymentMode", "Payment Mode", "Payment");

                let pBadge = `<span style="color: #10b981; font-size: 12px; font-weight: 600;">Paid</span>`;
                if (String(pMode).toLowerCase().includes("emi") || String(pMode).toLowerCase().includes("credit")) {
                    pBadge = `<span style="color: #f59e0b; font-size: 12px; font-weight: 600;">EMI / Pending</span>`;
                }

                return `<tr>
                    <td style="font-size: 13px;">${dStr}</td>
                    <td><span style="color: var(--primary); font-weight: 600;">${invNo || '-'}</span></td>
                    <td>${items || 'Unknown Item'}</td>
        ...
        tbody.innerHTML = '<tr><td colspan="5" style="text-align: center; color: var(--text-muted); padding: 20px;">Data not loaded yet.</td></tr>';
    }

    // Credit Block Display
    const emiBlock = document.getElementById('crmEmiBlock');
    if (c.tags.includes('Credit')) {
        emiBlock.style.display = 'block';
    } else {
...
// CRM Filter & Sort System
function runCrmFilters() {
    const searchInp = document.getElementById('crmSearchInput');
    const tagFilt = document.getElementById('crmTagFilter');
    const sortFilt = document.getElementById('crmSortFilter');
    if (!searchInp || !tagFilt || !sortFilt) return;

    let filtered = [...globalCrmData];
    const q = searchInp.value.toLowerCase().trim();
    const tag = tagFilt.value;
    const sort = sortFilt.value;
...
// Bind events for CRM controls
document.addEventListener("DOMContentLoaded", () => {
    const searchInp = document.getElementById('crmSearchInput');
    const tagFilt = document.getElementById('crmTagFilter');
    const sortFilt = document.getElementById('crmSortFilter');
    if (searchInp) searchInp.addEventListener('input', runCrmFilters);
    if (tagFilt) tagFilt.addEventListener('change', runCrmFilters);
    if (sortFilt) sortFilt.addEventListener('change', runCrmFilters);
});

// Helper to bridge Search results to CRM Profile
function jumpToCustomerProfile(name, phone) {
    const CRM_BTN = document.querySelector('[data-view="view-crm"]');
    if (CRM_BTN) {
        CRM_BTN.click(); // This triggers loadCRM() and view reset

        // Wait for potential async load
        setTimeout(() => {
            const id = (phone && phone !== "N/A") ? phone.trim() : name.trim();
            openCRMProfile(id);
        }, 800);
    }
}
...
// ========================================================
// EVO 4: NOTIFICATIONS SYSTEM (Low Stock & EMI Reminders)
// ========================================================
function checkNotifications() {
    ...
    if (typeof inventoryData !== 'undefined' && Array.isArray(inventoryData)) {
        const lowStock = inventoryData.filter(item => item.stock > 0 && item.stock <= 3);
        ...
        outOfStock.forEach(item => {
            notifs.push({
                ...
                icon: 'fa-box-open',
                title: 'Out of Stock!',
                desc: `${item.brand} ${item.model} is completely sold out.`
            });
        });
        ...
        lowStock.forEach(item => {
            notifs.push({
                ...
                desc: `Only ${item.stock} units left of ${item.brand} ${item.model}.`
            });
        });
    }

    // 2. EMI Reminders
    if (typeof globalAnalyticsData !== 'undefined' && Array.isArray(globalAnalyticsData)) {
        const emiSales = globalAnalyticsData.filter(row => {
            const mode = getInvVal(row, "paymentMode", "PaymentMode", "Payment Mode", "Payment");
            return String(mode).toLowerCase().includes("emi");
        });

        const today = new Date();
        emiSales.forEach(sale => {
            const pDateStr = sale["Purchase date"] || sale["Date"] || sale["timestamp"];
            if (!pDateStr) return;
            const pDate = parseSheetDate(pDateStr);
            const daysSince = Math.floor((today - pDate) / (1000 * 60 * 60 * 24));

            let tenure = Number(getInvVal(sale, "emiTenure", "Tenure")) || 6;
            let cName = getInvVal(sale, "Customer name", "Name", "customer") || "Customer";
            let phone = getInvVal(sale, "Phone number", "phone") || "";

            for (let month = 1; month <= tenure; month++) {
                let targetDays = month * 30; // approx 1 month
                let diff = targetDays - daysSince;
                ...
                if (diff >= 0 && diff <= 3) {
                    notifs.push({
                        ...
                        title: 'EMI Due Soon',
                        desc: `Month ${month} EMI for ${cName} (${phone}) is due in ${diff} days.`
                    });
                    ...
                    notifs.push({
                        ...
                        icon: 'fa-triangle-exclamation',
                        title: 'EMI Overdue!',
                        desc: `Month ${month} EMI for ${cName} is overdue by ${Math.abs(diff)} days.`
                    });
                }
            }
        });
    ...
    // Sort by type priority (danger first)
    notifs.sort((a, b) => {
        const p = { 'danger': 1, 'warning': 2, 'info': 3 };
    ...
    if (notifs.length === 0) {
        list.innerHTML = `
            <div style="text-align: center; color: var(--text-muted); padding: 40px 0;">
                <i class="fa-solid fa-check-circle" style="font-size: 32px; opacity: 0.3; margin-bottom: 12px;"></i><br>
                All caught up!
            </div>
        `;
        ...
        list.innerHTML = notifs.map(n => {
            // Mapping icon color based on type
            let color = 'var(--primary)';
            if (n.type === 'danger') color = '#ef4444';
            if (n.type === 'warning') color = '#f59e0b';
            if (n.type === 'info') color = '#3b82f6';

            return `
            <div class="notif-item notif-${n.type}">
                <i class="fa-solid ${n.icon} notif-icon" style="color: ${color};"></i>
...
// ========================================================
// SECTION 17 — SETTINGS MANAGEMENT (Save/Load)
// ========================================================
function loadSettings() {
    // Load Business Check
    const sn = document.getElementById("setShopName");
    if (sn) sn.value = localStorage.getItem("shopName") || "";

    const on = document.getElementById("setOwnerName");
    if (on) on.value = localStorage.getItem("ownerName") || "";

    const gn = document.getElementById("setGstNumber");
    if (gn) gn.value = localStorage.getItem("gstNumber") || "";

    const sa = document.getElementById("setShopAddress");
    if (sa) sa.value = localStorage.getItem("shopAddress") || "";

    // Load Invoice Configs Check
    const dg = document.getElementById("setDefaultGst");
    if (dg) dg.value = localStorage.getItem("defaultGst") || "18";

    const invF = document.getElementById("setInvoiceFormat");
    if (invF) invF.value = localStorage.getItem("invoiceFormat") || "simple";

    const invP = document.getElementById("setInvoicePrefix");
    if (invP) invP.value = localStorage.getItem("invoicePrefix") || "INV-";
...
function handleLogoUpload(event) {
    ...
    if (file.size > 2 * 1024 * 1024) {
        showToast("⚠ Logo too large! Max 2MB allowed.");
        return;
    }
...
function saveBusinessInfo() {
    localStorage.setItem("shopName", document.getElementById("setShopName").value.trim());
    localStorage.setItem("ownerName", document.getElementById("setOwnerName").value.trim());
    localStorage.setItem("gstNumber", document.getElementById("setGstNumber").value.trim());
    localStorage.setItem("shopAddress", document.getElementById("setShopAddress").value.trim());
    showToast("✔ Business Information Saved!");

    // If on Sales, clear the generateInvoiceID so prefix/name forces update on reload
    if (document.getElementById("invoiceID") && document.getElementById("invoiceID").value.includes("INV-")) {
...
function saveInvoiceSettings() {
    localStorage.setItem("defaultGst", document.getElementById("setDefaultGst").value);
    localStorage.setItem("invoiceFormat", document.getElementById("setInvoiceFormat").value);
    localStorage.setItem("invoicePrefix", document.getElementById("setInvoicePrefix").value.trim());

    // Attempt auto-re-roll of default ID if untouched
    const invEl = document.getElementById("invoiceID");
    if (invEl) {
        invEl.value = "";
        generateInvoiceID();
    ...
    const taxInp = document.getElementById("itemTaxRate");
    if (taxInp && (!taxInp.value || taxInp.value == "18")) taxInp.value = localStorage.getItem("defaultGst");
