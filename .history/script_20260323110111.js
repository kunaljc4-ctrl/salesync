...
// ========================================================
// EVO 4: QUICK SALE MODE
// ========================================================
async function submitQuickSale() {
    const btn = document.getElementById("qsSubmitBtn");
    if (btn.disabled) return;

    const cName = document.getElementById("qsCustomerName").value.trim();
    const cPhone = document.getElementById("qsCustomerPhone").value.trim();
    const product = document.getElementById("qsProduct").value.trim();
    const priceStr = document.getElementById("qsPrice").value.trim();
    const pMode = document.getElementById("qsPaymentMode").value;

    if (!cName || !cPhone || !product || !priceStr || !pMode) {
    ...
    const today = new Date();
    const datePart = today.getFullYear().toString() + String(today.getMonth() + 1).padStart(2, "0") + String(today.getDate()).padStart(2, "0");
    let counter = Number(localStorage.getItem("invoiceCounter")) || 1;
    const invId = `QS-${datePart}-${String(counter).padStart(4, "0")}`;

    const fmtDateStr = `${String(today.getDate()).padStart(2, '0')}/${String(today.getMonth() + 1).padStart(2, '0')}/${today.getFullYear()}`;

    // Construct Payload (Mimic submitToSheets payload)
    const payload = [{
        "Invoice ID": invId,
        "Customer name": cName,
        "Phone number": cPhone,
        "phone": cPhone,
        "Purchase date": fmtDateStr,
        "Notes": "Quick Sale",
        "Subtotal": price.toFixed(2),
        "Discount": "0",
        "Grand Total": price.toFixed(2),
        "Payment Mode": pMode,
        "Advance": "0",
        "Balance": "0",
        "Payment Status": "Paid",
        ...
        "category": "QuickSale",
        "brand": "Unknown" // Dummy values for analytics survival
    }];

    const completeSaleProcess = () => {
        let qsCount = Number(localStorage.getItem("invoiceCounter")) || 1;
        localStorage.setItem("invoiceCounter", qsCount + 1);

        document.getElementById('quickSaleForm').reset();
        showToast("✔ Quick Sale Completed!");
        btn.disabled = false;
    ...
    if (!navigator.onLine) {
        ...
        pending.push(payload);
        localStorage.setItem('pendingSales', JSON.stringify(pending));
        showToast("⚠ No Internet! Quick sale saved locally for auto-sync.");
        completeSaleProcess();
        return;
    }
    ...
    try {
        ...
        console.error("QS ERROR:", err);
        let pending = JSON.parse(localStorage.getItem('pendingSales') || '[]');
        pending.push(payload);
        localStorage.setItem('pendingSales', JSON.stringify(pending));
        showToast("❌ Server Failed! Quick sale saved locally.");
        completeSaleProcess();
    }
}
...
function openCRMProfile(index) {
    ...
    if (globalAnalyticsData && globalAnalyticsData.salesData) {
        const customerSales = globalAnalyticsData.salesData.filter(row => {
            ...
            return (rowPhone && rowPhone === c.phone) || (rowName && rowName.toLowerCase() === c.name.toLowerCase());
        });
        ...
        if (customerSales.length === 0) {
            ...
            tbody.innerHTML = customerSales.map(sale => {
                ...
                const invNo = getInvVal(sale, "Invoice Number", "Invoice No", "Invoice");
                const items = getInvVal(sale, "Products", "Product Details", "Items", "Brand");
