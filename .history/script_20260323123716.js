...
async function loadCRM() {
    ...
    try {
        ...
        if (data.success && data.results && data.results.length > 0) {
            ...
            data.results.forEach(row => {
                const cName = getInvVal(row, "Customer name", "Name", "Full Name", "customer");
                const phone = getInvVal(row, "Phone number", "phone", "Mobile", "WhatsApp");
                if (!cName && !phone) return;

                const id = phone || cName;
            ...
            // Generate automatic tags based on spend
...
function renderCRMTable(dataList) {
    ...
    tbody.innerHTML = dataList.map((c, i) => {
        ...
        const tagsHtml = c.tags.map(t => {
            if (t === 'VIP') return `<span style="background: rgba(16, 185, 129, 0.1); color: #10b981; padding: 4px 10px; border-radius: 100px; font-size: 11px; font-weight: 600;">VIP</span>`;
            if (t === 'EMI Active') return `<span style="background: rgba(245, 158, 11, 0.1); color: #f59e0b; padding: 4px 10px; border-radius: 100px; font-size: 11px; font-weight: 600;">EMI Active</span>`;
            if (t === 'New') return `<span style="background: rgba(59, 130, 246, 0.1); color: #3b82f6; padding: 4px 10px; border-radius: 100px; font-size: 11px; font-weight: 600;">New</span>`;
            return `<span style="background: rgba(100, 116, 139, 0.1); color: #64748b; padding: 4px 10px; border-radius: 100px; font-size: 11px; font-weight: 600;">${t}</span>`;
        }).join(" ");

        return `
        <tr style="cursor: pointer;" onclick="openCRMProfile(${i})">
            <td>
...
function openCRMProfile(index) {
    const c = globalCrmData[index];
    if (!c) return;
    ...
    // Tags
    const tagsHtml = c.tags.map(t => {
        if (t === 'VIP') return `<span style="background: rgba(16, 185, 129, 0.1); color: #10b981; padding: 4px 10px; border-radius: 100px; font-size: 11px; font-weight: 700;"><i class="fa-solid fa-crown"></i> VIP</span>`;
        if (t === 'EMI Active') return `<span style="background: rgba(245, 158, 11, 0.1); color: #f59e0b; padding: 4px 10px; border-radius: 100px; font-size: 11px; font-weight: 700;"><i class="fa-solid fa-hand-holding-dollar"></i> EMI</span>`;
        if (t === 'New') return `<span style="background: rgba(59, 130, 246, 0.1); color: #3b82f6; padding: 4px 10px; border-radius: 100px; font-size: 11px; font-weight: 700;">New</span>`;
        return `<span style="background: rgba(100, 116, 139, 0.1); color: #64748b; padding: 4px 10px; border-radius: 100px; font-size: 11px; font-weight: 700;">${t}</span>`;
    }).join(" ");
    ...
    const addr = recentSale ? getInvVal(recentSale, "Address", "Customer Address", "Location", "City", "Billing Address") : "";
    if (addressDiv) {
        addressDiv.innerHTML = `<i class="fa-solid fa-location-dot" style="width: 20px; color: var(--text-muted); text-align: center;"></i> ${addr || "No Address Provided"}`;
    ...
    if (c.tags.includes("VIP")) {
        generatedNote = "<strong>VIP Customer:</strong> High total spending, Frequent purchases, Pays on time";
    } else if (c.tags.includes("EMI Active")) {
        generatedNote = "<strong>Credit Customer:</strong> Has pending payments, Uses EMI";
    } else if (c.visits === 1) {
        generatedNote = "<strong>New Customer:</strong> First-time buyer, No history yet";
    } else if (daysSinceVisit > 90) {
        generatedNote = "<strong>Inactive Customer:</strong> Hasn't purchased in long time";
    } else if (c.totalSpend >= 15000 && c.visits <= 2) {
        generatedNote = "<strong>High-Value Customer:</strong> Spends a lot in one purchase, Even if not frequent";
    } else if (c.visits > 1 && c.visits <= 3) {
        generatedNote = "<strong>Returning Customer:</strong> Came back once or twice";
    } else {
        generatedNote = "<strong>Regular Customer:</strong> Buys occasionally, Average spending";
    }
