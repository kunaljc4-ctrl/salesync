...
// ========================================================
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbz19VGjj6wGg2n02sldeEjwNlvDKfjBO8qGSBXuiI7CNIM0ahrKGEKHH-vi0M4Nj-nqMA/exec"
...
async function performServerCustomerLookup(phone) {
    try {
        ...
        if (text && text.trim().startsWith("{")) {
            ...
            const mapped = {
                customerName: getInvVal(customer, "Customer name", "Customer Name", "Name", "Full Name", "customerName", "name"),
                customerPhone: getInvVal(customer, "Phone number", "Phone Number", "Phone", "phone", "customerPhone") || phone,
                altPhone: getInvVal(customer, "altPhone", "Alternate Phone", "Alt Phone", "Alt_Phone", "alt_phone", "AlternatePhone"),
                customerEmail: getInvVal(customer, "customerEmail", "Email", "Email Address", "Email_Address", "email"),
                customerAddress: getInvVal(customer, "customerAddress", "Address", "Permanent address", "Billing Address", "address", "Permanent_address")
            };
            autoFillCustomer(mapped);
            return true;
        } else {
            console.warn("Lookup result:", text);
            if (text.includes("Customer Not Found")) {
                showToast("ℹ Customer not found in database");
            } else if (text.includes("missing")) {
                showToast("⚠ Sheet headers mismatch: " + text);
            }
            return false;
        }
    } catch (err) {
        console.error("Server lookup error:", err);
        // showToast("❌ Database connection error"); // Don't spam if just offline
        return false;
    }
}
            ...
            data.results.forEach(row => {
                ...
                if (!invoices[invId]) {
                    invoices[invId] = {
                        ...
                        altPhone: row["altPhone"] || row["Alternate Phone"] || "",
                        customerEmail: row["customerEmail"] || row["Email"] || "",
                        customerAddress: row["customerAddress"] || row["Address"] || row["Permanent address"] || "",
