...
// ========================================================
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbzmhUef41YC5wTueXB-GULv88R81st0Q2GbsOvntEJOJ_eNg333roXnDfKcgFrqH71wmQ/exec"
...
function updatePricing() {
    ...
    const totalFinalPrice = baseAfterDiscount + totalTaxAmount;
...
function togglePaymentFields() {
    ...
    if (pMode === "") {
    ...
    } else if (pMode === "Credit (Udhaar)" || pMode === "EMI Finance") {
        if (paymentStatusField) paymentStatusField.style.display = "none";
        ...
        if (advanceField) {
            ...
            const advInput = document.getElementById("advance");
            if (advInput) advInput.value = ""; // Clear when hidden
        }
        if (balanceField) balanceField.style.display = "none";
    }
    updatePricing();
...
if (customerPhone) customerPhone.addEventListener("input", sanitizePhone);
if (altPhone) altPhone.addEventListener("input", sanitizePhone);

if (customerPhone) {
    customerPhone.addEventListener("input", async () => {
        const phone = customerPhone.value.trim();
        if (phone.length === 10) {
            try {
                showToast("🔍 Looking up customer...");
                const response = await fetch(`${SCRIPT_URL}?phone=${phone}`);
                const text = await response.text();

                if (text && text.startsWith("{")) {
                    const customer = JSON.parse(text);
                    // Using precise header names returned from GAS
                    document.getElementById("customerName").value = customer["Customer name"] || customer.name || "";
                    document.getElementById("customerEmail").value = customer.customerEmail || customer.email || "";
                    document.getElementById("customerAddress").value = customer.customerAddress || customer.address || "";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (navAnalytics) navAnalytics.addEventListener("click", updateDashboard);
});
            ...
            data.results.forEach(row => {
                ...
                if (!invoices[invId]) {
                    invoices[invId] = {
                        ...
                        customerName: row["Customer name"] || row["Name"] || "Unknown",
                        customerPhone: row["Phone number"] || row["phone"] || row["Phone"] || "",
        ...
        .then(data => {
            if (data.success && data.results) {
                ...
                for (let i = 29; i >= 0; i--) {
                    ...
                    timelineMap[dayKey] = 0;
                }

                data.results.forEach(row => {
                    let rev = Number(row["Revenue_Pricing"] || 0);
                    let prof = Number(row["Profit_Pricing"] || 0);
                    let cat = row["Category_Product"] || "Other";
                    let dateStr = row["Purchase date"] || row["Date"] || new Date().toISOString();
                    ...
                    let invId = row["Invoice ID"] || row["ID"];

                    totalRev += rev;
                    totalProf += prof;
                    if (invId) uniqueInvoices.add(invId);

                    categoryMap[cat] = (categoryMap[cat] || 0) + 1;

                    const dayKey = dateStr.split('T')[0];
                    if (timelineMap.hasOwnProperty(dayKey)) {
                        timelineMap[dayKey] += rev;
                    }
                });
                ...
                document.getElementById("dash-total-sales").innerText = uniqueInvoices.size || data.results.length;

                const categories = Object.keys(categoryMap);
                const categoryCounts = Object.values(categoryMap);

                const timelineLabels = Object.keys(timelineMap).map(d => new Date(d).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }));
                const timelineData = Object.values(timelineMap);

                renderSalesChart(timelineLabels, timelineData);
                renderCategoryChart(categories, categoryCounts);
            }
        })
        .catch(err => {
            ...
            const dashTotalRev = document.getElementById("dash-total-revenue");
            if (dashTotalRev) dashTotalRev.innerText = "Error";
        });
}
