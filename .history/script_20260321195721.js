...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (analyticsSidebarToggle) {
        analyticsSidebarToggle.addEventListener("click", () => {
            const sidebarBtn = document.getElementById("appSidebar");
            if (sidebarBtn) sidebarBtn.classList.toggle("collapsed");
        });
    }
});
...
function populateAnalyticsNav() {
    ...
    const uniqueSegments = [...new Set(globalAnalyticsData.map(r => r.paymentMode || r.PaymentMode || "Standard"))].filter(Boolean);
...
function updateNavActiveStates() {
    ...
    document.querySelectorAll('#analytics-months-list .nav-item').forEach(btn => {
        btn.classList.toggle('active', filteredMonth === months.indexOf(btn.innerText));
    });
    // Segments
...
function processAndRenderAnalytics() {
    ...
    const filteredResults = globalAnalyticsData.filter(row => {
        if (filteredSegment) {
            const seg = row["paymentMode"] || row["PaymentMode"] || "Standard";
            if (seg !== filteredSegment) return false;
        }

        let dateStr = row["Purchase date"] || row["Date"];
    ...
    filteredResults.forEach(row => {
        ...
        let qty = Number(row["Qty"] || row["Quantity"] || 1);
        let brand = row["Brand"] || row["brand"] || "Unknown";
        let segment = row["paymentMode"] || row["PaymentMode"] || "Standard";
        let product = row["Product_Name"] || row["Product"] || "Unknown";
...
function renderAdvancedCharts(cData) {
    ...
    // 2. Revenue by Payments (Doughnut)
